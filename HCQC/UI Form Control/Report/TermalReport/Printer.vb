...
#End Region
        ...
        Public Shared Function SendStringToPrinter(ByVal printerName As String, ByVal stringToPrint As String) As Boolean
            Try
                ...
                success = SendBytesToPrinter(printerName, memoryPointer, bufferSize)
                Marshal.FreeCoTaskMem(memoryPointer)
                Console.WriteLine(Application.StartupPath & "\hcqclog.txt")
                If Not success Then
                    printerError = Marshal.GetLastWin32Error()
                    file = My.Computer.FileSystem.OpenTextFileWriter(Application.StartupPath & "\hcqclog.txt", True)
                    file.WriteLine(login.Luserid.Text & "#" & "Error Printer :" & printerError & "#" & GetIPAddress() & "#" & Now())
                    file.Close()
                End If
