...
Public Class DayaTumbuhRM
    ...
    Private Sub DayaTumbuhRM_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        Me.Dispose()
    End Sub

    Private Sub DayaTumbuhRM_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Private Sub btnCari2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCari2.Click
        Dim sql As String = ""
        openDB()
        sql = "Select * From rm_vg WHERE (no_lab= '" & tnolab.Text & "') and (char1s='Normal Seedling (%)')"
        ...
        If dread.Read = True Then
            ...
            con.Close()
            openDB()
            sql = "Select * From rm_vg WHERE (no_lab= '" & tnolab.Text & "') and (char1s='Abnormal Seedling (%)')"
            cmd = New SqlClient.SqlCommand(sql, con)
            cmd.CommandType = CommandType.Text
            cmd.CommandText = sql
            dread = cmd.ExecuteReader
            ...
            con.Close()
            openDB()
            sql = "Select * From rm_vg WHERE (no_lab= '" & tnolab.Text & "') and (char1s='Hard Seed (%)')"
            cmd = New SqlClient.SqlCommand(sql, con)
            cmd.CommandType = CommandType.Text
            cmd.CommandText = sql
            dread = cmd.ExecuteReader
            ...
            con.Close()
            openDB()
            sql = "Select * From rm_vg WHERE (no_lab= '" & tnolab.Text & "') and (char1s='Fresh Seed (%)')"
            cmd = New SqlClient.SqlCommand(sql, con)
            cmd.CommandType = CommandType.Text
            cmd.CommandText = sql
            dread = cmd.ExecuteReader
            ...
            con.Close()
            openDB()
            sql = "Select * From rm_vg WHERE (no_lab= '" & tnolab.Text & "') and (char1s='Dead Seed (%)')"
            cmd = New SqlClient.SqlCommand(sql, con)
            cmd.CommandType = CommandType.Text
            cmd.CommandText = sql
            dread = cmd.ExecuteReader
            ...
            con.Close()
            openDB()
            sql = "Select * From rm_vg WHERE (no_lab= '" & tnolab.Text & "') and (char1s='Index Vigor (%)')"
            cmd = New SqlClient.SqlCommand(sql, con)
            cmd.CommandType = CommandType.Text
            cmd.CommandText = sql
            dread = cmd.ExecuteReader
            ...
            con.Close()
            openDB()
            sql = "Select * From rm_vg WHERE (no_lab= '" & tnolab.Text & "') and (char1s='Germination (%)')"
            cmd = New SqlClient.SqlCommand(sql, con)
            cmd.CommandType = CommandType.Text
            cmd.CommandText = sql
            dread = cmd.ExecuteReader
            ...
            If dread.Read = True Then
                ...
                tnojob.Text = dread.Item("farmer")
                tnomnl.Text = dread.Item("lokasi")
    ...
    Private Sub btncari1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btncari1.Click
        Dim sql As String = ""
        openDB()
        sql = "Select * From rm_vg WHERE (no_lab= '" & tnolab.Text & "')"
        cmd = New SqlClient.SqlCommand(sql, con)
        cmd.CommandType = CommandType.Text
        cmd.CommandText = sql
        dread = cmd.ExecuteReader
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If dread.Read = False Then
            ...
            Else
                ...
                _RunSQL("Update rm_msk SET vg_Dt_t ='" & CDate(tgl_uji.Text) & "', vg_fs_c ='" & CDate(tgl_fs.Text) & "', vg_fn_c ='" & CDate(tgl_fn.Text) & "', vg_analys ='" & tanalys.Text & "', vg_mtd ='" & mtd & "', vg_cek ='" & ck & "', vg_tgl_input='" & Now & "' WHERE (no_lab = '" & tnolab.Text & "')")
                ...
                For values As Integer = 1 To 7
                    If values = 1 Then
                        'NRML
                        sqlvg = "Insert Into rm_vg(no_lab,char1s,t1,t2,t3,t4,avrg,percn) values('" & tnolab.Text & "','Normal Seedling (%)','" & Val(tNml1.Text) & "','" & Val(tNml2.Text) & "','" & Val(tNml3.Text) & "','" & Val(tNml4.Text) & "','" & Val(tNmlArg.Text) & "','" & Val(tNmlPrc.Text) & "')"
                        cmdvg = New SqlClient.SqlCommand(sqlvg, con)
                        xvg = cmdvg.ExecuteNonQuery
                    ElseIf values = 2 Then
                        'AB NORML
                        sqlvg = "Insert Into rm_vg(no_lab,char1s,t1,t2,t3,t4,avrg,percn) values('" & tnolab.Text & "','Abnormal Seedling (%)','" & Val(tAbNml1.Text) & "','" & Val(tAbNml2.Text) & "','" & Val(tAbNml3.Text) & "','" & Val(tAbNml4.Text) & "','" & Val(tAbNmlArg.Text) & "','" & Val(tAbNmlPrc.Text) & "')"
                        cmdvg = New SqlClient.SqlCommand(sqlvg, con)
                        xvg = cmdvg.ExecuteNonQuery
                    ElseIf values = 3 Then
                        ''VIGOR
                        sqlvg = "Insert Into rm_vg(no_lab,char1s,t1,t2,t3,t4,avrg,percn) values('" & tnolab.Text & "','Index Vigor (%)','" & Val(tVg1.Text) & "','" & Val(tVg2.Text) & "','" & Val(tVg3.Text) & "','" & Val(tVg4.Text) & "','" & Val(tVgArg.Text) & "','" & Val(tVgPrc.Text) & "')"
                        cmdvg = New SqlClient.SqlCommand(sqlvg, con)
                        xvg = cmdvg.ExecuteNonQuery
                    ElseIf values = 4 Then
                        'HARD SED
                        sqlvg = "Insert Into rm_vg(no_lab,char1s,t1,t2,t3,t4,avrg,percn) values('" & tnolab.Text & "','Hard Seed (%)','" & Val(tHd1.Text) & "','" & Val(tHd2.Text) & "','" & Val(tHd3.Text) & "','" & Val(tHd4.Text) & "','" & Val(tHdArg.Text) & "','" & Val(tHdPrc.Text) & "')"
                        cmdvg = New SqlClient.SqlCommand(sqlvg, con)
                        xvg = cmdvg.ExecuteNonQuery
                    ElseIf values = 5 Then
                        'FRESS
                        sqlvg = "Insert Into rm_vg(no_lab,char1s,t1,t2,t3,t4,avrg,percn) values('" & tnolab.Text & "','Fresh Seed (%)','" & Val(tFrs1.Text) & "','" & Val(tFrs2.Text) & "','" & Val(tFrs3.Text) & "','" & Val(tFrs4.Text) & "','" & Val(tFrsArg.Text) & "','" & Val(tFrsPrc.Text) & "')"
                        cmdvg = New SqlClient.SqlCommand(sqlvg, con)
                        xvg = cmdvg.ExecuteNonQuery
                    ElseIf values = 6 Then
                        'DEAD
                        sqlvg = "Insert Into rm_vg(no_lab,char1s,t1,t2,t3,t4,avrg,percn) values('" & tnolab.Text & "','Dead Seed (%)','" & Val(tDd1.Text) & "','" & Val(tDd2.Text) & "','" & Val(tDd3.Text) & "','" & Val(tDd4.Text) & "','" & Val(tDdArg.Text) & "','" & Val(tDdPrc.Text) & "')"
                        cmdvg = New SqlClient.SqlCommand(sqlvg, con)
                        xvg = cmdvg.ExecuteNonQuery
                    ElseIf values = 7 Then
                        'GERMIN
                        sqlvg = "Insert Into rm_vg(no_lab,char1s,t1,t2,t3,t4,avrg,percn) values('" & tnolab.Text & "','Germination (%)','" & Val(tGm1.Text) & "','" & Val(tGm2.Text) & "','" & Val(tGm3.Text) & "','" & Val(tGm4.Text) & "','" & Val(tGmArg.Text) & "','" & Val(tGmPrc.Text) & "')"
                        cmdvg = New SqlClient.SqlCommand(sqlvg, con)
                        xvg = cmdvg.ExecuteNonQuery
                    End If
                Next
    ...
    Private Sub btnEdit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnEdit.Click
        ...
        Else
            ...
            For values As Integer = 1 To 7
                If values = 1 Then
                    sqlvg = "Update rm_vg Set no_lab='" & tnolab.Text & "', t1='" & Val(tNml1.Text) & "',t2='" & Val(tNml2.Text) & "',t3='" & Val(tNml3.Text) & "',t4='" & Val(tNml4.Text) & "',avrg='" & Val(tNmlArg.Text) & "',percn='" & Val(tNmlPrc.Text) & "' Where (no_lab='" & tnolab.Text & "') And (char1s='Normal Seedling (%)')"
                    cmdvg = New SqlClient.SqlCommand(sqlvg, con)
                    xvg = cmdvg.ExecuteNonQuery
                ElseIf values = 2 Then
                    sqlvg = "Update rm_vg Set no_lab='" & tnolab.Text & "', t1='" & Val(tAbNml1.Text) & "',t2='" & Val(tAbNml2.Text) & "',t3='" & Val(tAbNml3.Text) & "',t4='" & Val(tAbNml4.Text) & "',avrg='" & Val(tAbNmlArg.Text) & "',percn='" & Val(tAbNmlPrc.Text) & "' Where (no_lab='" & tnolab.Text & "') And (char1s='Abnormal Seedling (%)')"
                    cmdvg = New SqlClient.SqlCommand(sqlvg, con)
                    xvg = cmdvg.ExecuteNonQuery
                ElseIf values = 3 Then
                    sqlvg = "Update rm_vg Set no_lab='" & tnolab.Text & "', t1='" & Val(tVg1.Text) & "',t2='" & Val(tVg2.Text) & "',t3='" & Val(tVg3.Text) & "',t4='" & Val(tVg4.Text) & "',avrg='" & Val(tVgArg.Text) & "',percn='" & Val(tVgPrc.Text) & "' Where (no_lab='" & tnolab.Text & "') And (char1s='Index Vigor (%)')"
                    cmdvg = New SqlClient.SqlCommand(sqlvg, con)
                    xvg = cmdvg.ExecuteNonQuery
                ElseIf values = 4 Then
                    sqlvg = "Update rm_vg Set no_lab='" & tnolab.Text & "', t1='" & Val(tHd1.Text) & "',t2='" & Val(tHd2.Text) & "',t3='" & Val(tHd3.Text) & "',t4='" & Val(tHd4.Text) & "',avrg='" & Val(tHdArg.Text) & "',percn='" & Val(tHdPrc.Text) & "' Where (no_lab='" & tnolab.Text & "') And (char1s='Hard Seed (%)')"
                    cmdvg = New SqlClient.SqlCommand(sqlvg, con)
                    xvg = cmdvg.ExecuteNonQuery
                ElseIf values = 5 Then
                    sqlvg = "Update rm_vg Set no_lab='" & tnolab.Text & "', t1='" & Val(tFrs1.Text) & "',t2='" & Val(tFrs2.Text) & "',t3='" & Val(tFrs3.Text) & "',t4='" & Val(tFrs4.Text) & "',avrg='" & Val(tFrsArg.Text) & "',percn='" & Val(tFrsPrc.Text) & "' Where (no_lab='" & tnolab.Text & "') And (char1s='Fresh Seed (%)')"
                    cmdvg = New SqlClient.SqlCommand(sqlvg, con)
                    xvg = cmdvg.ExecuteNonQuery
                ElseIf values = 6 Then
                    sqlvg = "Update rm_vg Set no_lab='" & tnolab.Text & "', t1='" & Val(tDd1.Text) & "',t2='" & Val(tDd2.Text) & "',t3='" & Val(tDd3.Text) & "',t4='" & Val(tDd4.Text) & "',avrg='" & Val(tDdArg.Text) & "',percn='" & Val(tDdPrc.Text) & "' Where (no_lab='" & tnolab.Text & "') And (char1s='Dead Seed (%)')"
                    cmdvg = New SqlClient.SqlCommand(sqlvg, con)
                    xvg = cmdvg.ExecuteNonQuery
                ElseIf values = 7 Then
                    sqlvg = "Update rm_vg Set no_lab='" & tnolab.Text & "', t1='" & Val(tGm1.Text) & "',t2='" & Val(tGm2.Text) & "',t3='" & Val(tGm3.Text) & "',t4='" & Val(tGm4.Text) & "',avrg='" & Val(tGmArg.Text) & "',percn='" & Val(tGmPrc.Text) & "' Where (no_lab='" & tnolab.Text & "') And (char1s='Germination (%)')"
                    cmdvg = New SqlClient.SqlCommand(sqlvg, con)
                    xvg = cmdvg.ExecuteNonQuery
                End If
            Next
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        If result = DialogResult.Yes Then
            _RunSQL("DELETE FROM rm_vg WHERE(no_lab = '" & tnolab.Text & "')")
            _RunSQL("Update rm_msk SET vg_Dt_t =NULL, vg_fs_c =NULL, vg_fn_c =NULL, vg_analys =NULL, vg_mtd ='', vg_cek =NULL, vg_tgl_input=NULL WHERE (no_lab = '" & tnolab.Text & "')")
            Me.tnolab.Focus()
            'MetroClear()
        ElseIf result = DialogResult.No Then
            btnSave.Enabled = True
